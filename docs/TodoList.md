...
> P4-1 已完成，见 7.10；P4Q-2 批 2a/2b 与 P4Q-1 剩余上下文已完成，见 7.11 / 7.12。
> P4Q-3b 与 P4-8a 构成后续纵向交付目标。
...
- [ ] **P4Q-2 覆盖与时间驱动的计划和状态**
  ...
  - 剩余：「充分回答可转场 / 无新增信息可收束」的语义判定属 P4Q-3b（一次逐轮语义调用），
    本批只落了 Java 硬边界（必要覆盖完成 / 预算用尽即收束）；
    覆盖证据的展示明细（每个话题的覆盖状态卡）与 Agent 提案契约属 P4-8a。
