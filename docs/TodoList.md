...
> P4-1、P4Q-2、P4Q-1 剩余上下文、P4Q-3b 与 P4-8a 构成一个纵向交付目标。
> 可以按数据模型 → 决策 → 前端接入拆分改动，但字段必须有真实生产者与消费者，不把空壳字段视为功能完成。

- [ ] **P4-1 候选素材与实际轮次分离**
  - 候选使用稳定题目标识、父子关系、经历 / 话题关联与技能标识；实际展示、作答、评估与最终决定独立保存。
  - 区分「项目经历」等交流话题与「Java」等考察技能，不再由一个 category 同时表达两种含义。
  - 数组索引不再决定面试路径和结束条件；候选耗尽不自动等于考察完成，缺少候选时按覆盖与预算给出明确处理结果。
  - 保留 difficulty、expectedPoints、followUpType 等元数据；优先扩展现有存储，不强制引入独立题库服务或复杂图数据库。
  - 验收：未问候选不进入实际轨迹，问题顺序变化不破坏恢复、证据关联和报告；旧会话可回放与继续。
- [ ] **P4Q-2 覆盖与时间驱动的计划和状态**
