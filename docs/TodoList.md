...
- [ ] **P4Q-1 简历驱动的面试上下文**
  ...
  - [ ] **逐轮上下文供给（批 3）**
    - 现在可做（有数据支撑）：本场**简历相关片段**（取自 P4Q-1a 落下的会话快照）、
      **最近相关问答**（同技能/同话题最近若干轮，来自题单与作答记录）、**画像基线**（该技能当前分与证据条数）。
    - **依赖 4.3 的两项不在批 3 做**：**覆盖摘要**需要覆盖证据的存储、**剩余预算**需要时间预算概念，
      两者都是 P4Q-2 的产物。先落空壳字段只会得到永远为空的 prompt 变量——`focusCategories` 曾这样变成
      纯装饰（界面写「重点考察 JVM」，实际照旧问 MySQL），这类「看起来生效」的坑不再造一遍。
      P4Q-2 落地后作为批 3 的追加项补齐。
    - 输入预算（先给默认值，实测后调）：简历片段 ≤ 800 字、最近问答 ≤ 2 轮。逐轮上下文变长会直接顶到
      P95 ≤ 3s 的目标，因此预算与裁剪都在 Java 侧算（Java 是 Engine 与事实源，Python 不参与逐轮）。
    - 自证方式：**上下文构造测试**（断言 prompt 变量内容与裁剪边界），不断言模型行为——
      模型输出无法稳定断言，能稳定断言的是「我们喂进去了什么」。
    - 边界：本批只备齐上下文，**不合并模型调用、不引入预算语义**（那是 P4Q-2 / P4Q-3）。
